...
uint64_t hash(char *data, size_t length) {
...
uint64_t hash_fnv1(char *data, size_t length) {
...
uint64_t hash_fnv1a(char *data, size_t length) {
  uint64_t hash_value = FNV_OFFSET;
  for (int i = 0; i < length; i++) {
    hash_value ^= data[i];
    hash_value *= FNV_PRIME;
...
int main(int argc, char **argv) {
  ...
  hash_table *table = hash_table_create(tablesize, hash_fnv1a);
  ...
  fclose(fp);
  printf("Loading %d word(s) into the table\n", numwords);
